...
public class ModernEncryptionController {
    ...
    private void setupTableColumns() {
        ...
        nameCol.setPrefWidth(200); // 열 너비를 150픽셀로 고정 (원하는 값으로 변경 가능)
    ...
    private void onCreateKey() {
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == ButtonType.OK) {
                ...
                // 취소 버튼을 눌렀을 때 명시적으로 아무것도 반환하지 않음
                return null;
            }
        ...
        currentTask = new Task<>() {
            ...
            protected Void call() throws Exception {
                ...
                if (total == 1 && !new File(currentDirectory, selectedItems.get(0).getName()).isDirectory()) {
                    final FileItem item = selectedItems.get(0); // final로 선언
                    final File file = new File(currentDirectory, item.getName()); // final로 선언
                    ...
                    try {
                        final File tempDecryptedFinal = new File(currentDirectory, "temp_" + item.getName()); // final로 선언
    ...
    private void onDecrypt() {
        ...
        currentTask = new Task<>() {
            @Override
            protected Void call() throws Exception {
                final int total = encryptedFiles.size(); // total을 final로 선언
                for (int i = 0; i < total; i++) {
                    final FileItem item = encryptedFiles.get(i); // final로 선언
                    final File file = new File(currentDirectory, item.getName()); // final로 선언
                    final String outputPath = generateUniqueOutputPath(file.getPath().substring(0, file.getPath().length() - 5)); // final로 선언
                    final int currentIndex = i; // i를 final 복사본으로 생성

                    Future<?> future = executor.submit(() -> {
                        try {
                            updateProgress(currentIndex, total); // final 복사본 사용
